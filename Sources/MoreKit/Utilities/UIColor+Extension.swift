...
extension UIColor {
    ...
    public func generateLightDarkString(_ userInterfaceStyle: UIUserInterfaceStyle? = nil) -> String {
...
extension UIColor {
    public var isLight: Bool {
        ...
        let threshold: CGFloat = 75.0
        let gray = ColorCalculator.YtoLstar(Y: ColorCalculator.rgbToY(r: red, g: green, b: blue))
        return gray > threshold
    }

    public func isSimilar(to color: UIColor, threshold: CGFloat = 0.2) -> Bool {
...
extension UIColor {
    public func overlay(on background: UIColor) -> UIColor {
...
public struct ColorCalculator {
    public static func sRGBtoLin(_ colorChannel: CGFloat) -> CGFloat {
        colorChannel <= 0.04045 ? colorChannel / 12.92 : pow((colorChannel + 0.055) / 1.055, 2.4)
    }

    public static func rgbToY(r: CGFloat, g: CGFloat, b: CGFloat) -> CGFloat {
        0.2126 * sRGBtoLin(r) + 0.7152 * sRGBtoLin(g) + 0.0722 * sRGBtoLin(b)
    }

    public static func YtoLstar(Y: CGFloat) -> CGFloat {
